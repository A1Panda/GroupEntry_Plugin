...
function getJoinGroupData() {
  try {
    ...
    const joinGroups = Object.entries(data).map(([groupId, config]) => {
      ...
      // ans 兼容字符串数组和对象数组，统一输出字符串数组
      let ansArr = []
      if (Array.isArray(config.ans)) {
        if (typeof config.ans[0] === 'object') {
          ansArr = config.ans.map(item => item.value || '')
        } else {
          ansArr = config.ans.map(value => String(value))
        }
      }
      // BlackList 兼容字符串数组和对象数组，统一输出字符串数组
      let blackArr = []
      if (Array.isArray(config.BlackList)) {
        if (typeof config.BlackList[0] === 'object') {
          blackArr = config.BlackList.map(item => item.userId || '')
        } else {
          blackArr = config.BlackList.map(userId => String(userId))
        }
      }
      const groupConfig = {
        groupId: groupIdStr,
    ...
    logger.debug('[群组邀请管理] 转换后的 joinGroups 数据:', JSON.stringify(joinGroups))
    return { joinGroups }
  } catch (err) {
...
function setJoinGroupData(data) {
  try {
    logger.debug('[群组邀请管理] setJoinGroupData 入参:', JSON.stringify(data))
    // 验证数据格式
    ...
    data.joinGroups.forEach(group => {
      if (group && group.groupId) {
        ...
        config[groupIdStr] = {
          wenti: group.wenti || '',
          // ans 只保存为字符串数组
          ans: Array.isArray(group.ans) ? group.ans.map(item => String(item)).filter(Boolean) : [],
          // BlackList 只保存为字符串数组
          BlackList: Array.isArray(group.BlackList) ? group.BlackList.map(item => String(item)).filter(Boolean) : [],
    ...
    try {
      ...
      logger.debug('[群组邀请管理] 实际保存后的 JoinGroup.json 数据:', JSON.stringify(savedData))
